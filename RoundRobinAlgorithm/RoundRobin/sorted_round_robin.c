...
/**
* Same as finding mean.
*/
float sorted_time_quantum(ReadyProcess* readyQueue, int readyQueueSize)
{
    float sum = 0;
    int count = 0;
...
RoundRobinResult sorted_round_robin(Process *processes, int processesSize, char* grouping) {
RoundRobinResult result;
    ...
    while (1)
    {
        ...
        if (processHadArrivedFlag == 1){
            float newTimeQuantum = sorted_time_quantum(readyQueue, readyQueueSize);
